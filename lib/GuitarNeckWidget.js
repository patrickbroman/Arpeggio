...
GuitarNeckWidget.prototype.setNoteStateForWidget = function(widget, state) {
    widget.backgroundColor = NOTE_STATE_COLORS[state];
    widget.fontOpacity = NOTE_STATE_FONTOPACITY[state];
    widget.noteState = state;

}

GuitarNeckWidget.prototype.setNoteStateForAll = function(state) {
    for(var i = 0; i < this.noteWidgets.length; i++) {
            this.setNoteStateForWidget(this.noteWidgets[i], state);
    }
}
...
GuitarNeckWidget.prototype.setNoteStateForStringAndFret = function(string, fret, state) {
    ...
    this.setNoteStateForWidget(this.noteWidgets[idx], state);
}

GuitarNeckWidget.prototype.contains = function(x, y) {
    return true; // FIXME
}
...
GuitarNeckWidget.prototype.mouseDown = function(x, y) {
    for(var i = 0; i < this.noteWidgets.length; i++) {
        var w = this.noteWidgets[i];
        if(w.contains(x, y)) {
            w.mouseDown(x, y);
            this.setNoteStateForWidget(w, w.noteState+1);
        }
    }
