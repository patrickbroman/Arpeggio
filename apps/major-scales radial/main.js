...
function makeWidgets(key, yOff, tOff) {
    ...
    for(var i = 0; i < 12; i++) {
        let ang = i * Math.PI * 2.0 / 12.0;

        var xPos = 1920/2 + 260 * Math.cos(ang-Math.PI/2.0);
        var yPos = 1080/2 + 260 * Math.sin(ang-Math.PI/2.0);

        circPositions.push({x: xPos, y: yPos});
    }
    ...
    for(var i = 0; i < 12; i++) {
        ...
        for(var j = 0; j < 7; j++) {
            noteWidget.addAnimation(makeAnimation(
                {
                    "x" : makeInterpolator(
                        circPositions[mod(i-j, 12)].x, 
                        circPositions[mod(i-j-1, 12)].x
                    ),
                    "y" : makeInterpolator(
                        circPositions[mod(i-j, 12)].y, 
                        circPositions[mod(i-j-1, 12)].y
                    ),
                }
            )
            , 500 + j*200, 560 + j*200);
